...
def insert_denoiser(model, quantize_index, strategy, inchannel_list):
    assert len(quantize_index) == len(strategy), \
        'You should provide the same number of bit setting as layer list!'
    for i, value in enumerate(quantize_index):
        if strategy[i] == 1:
            denoise = Denoising(inchannel_list[i])
            denoise.cuda()
            model.update(i, denoise)
    print("denoiser insert successfully!", strategy)
    return model

def remove_denoiser(model, quantize_index, strategy):
    if strategy != []:
        for i, value in enumerate(quantize_index):
            if strategy[i] == 1:
                model.remove(i)
        print("denoiser remove successfully!")
